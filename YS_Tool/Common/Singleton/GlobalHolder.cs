...
namespace YS_Tool.Common.Singleton {
    public class GlobalHolder : MonoBehaviour {
        ...
        private void Awake() {
            if (CheckExistHolder()) {
#if DEBUG_ON
                Debug.LogFormat("<color=red>{0} is already exist!</color>", typeof(GlobalHolder).Name);
				Debug.LogFormat("<color=red>    And Destroy object where the name is {0}</color>", gameObject.name);
#endif
                Destroy(gameObject);
            } else {
#if DEBUG_ON
                Debug.LogFormat("<color=blue>{0} is spawn!</color>", typeof(GlobalHolder).Name);
#endif
                InitHolder();
            }
        }
...
#if DEBUG_ON
            Debug.LogFormat("<color=blue>{0} is destroyed!</color>", typeof(GlobalHolder).Name);
#endif
        }
        ...
        private void InitHolder() {
            gameObject.name = typeof(GlobalHolder).Name;
            m_Holder = gameObject;
