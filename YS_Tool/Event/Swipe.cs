...
namespace YS_Tool.Event {
    ...
    public class Swipe : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler {
        ...
        public void OnBeginDrag(PointerEventData e) {
            ...
            if (m_IsPressing || m_IsMoveToStart) return;
        ...
        private void CallSwipeEvent() {
            ...
            if (dy < dx) {
                if (requiredPx < dx) m_SwipeDir = Mathf.Sign(dist.x) < 0 ? Vector2.left : Vector2.right;
            } else {
                if (requiredPx < dy) m_SwipeDir = Mathf.Sign(dist.y) < 0 ? Vector2.down : Vector2.up;
            }
