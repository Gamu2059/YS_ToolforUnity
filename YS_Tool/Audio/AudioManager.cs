...
namespace YS_Tool.Audio {
    /// <summary>
    /// オーディオの再生などを管理する。
    /// 一点からの再生になるため、スマホアプリ用。
    /// </summary>
    public class AudioManager<T> : LocalSingletonMonoBehavior<T> where T : MonoBehaviour {
        ...
        public AudioMixerGroup BgmMixer {
            get { return m_BgmMixer; }
        }

        public AudioMixerGroup SeMixer {
            get { return m_SeMixer; }
        }

        public List<AudioInfo> AudioInfos {
            get { return m_AudioInfos; }
        }
        ...
        public AudioClip GetAudioClip(string audioClipName) {
            ...
            for (int i = 0; i < m_AudioInfos.Count; i++) {
                ...
                if (audioClipName == info.Name) return info.Clip;
            }
        ...
        public void PlayBGM(AudioClip bgmClip) {
            ...
            // 空いているソースがあればそれで再生
            for (int i = 0; i < m_BgmSources.Length; i++) {
                if (!CheckExistBGMSource(i)) continue;
        ...
        public void StopBGM(AudioClip bgmClip) {
            ...
            for (int i = 1; i < m_BgmSources.Length; i++) {
                ...
                var src = m_BgmSources[i];
                if (src.clip != bgmClip || !src.isPlaying) continue;
                src.Stop();
            }
        }
        ...
        public bool IsPlayingBGM(AudioClip bgmClip) {
            if (!bgmClip) return false;

            for (int i = 0; i < m_BgmSources.Length; i++) {
                if (!CheckExistBGMSource(i)) continue;

                var src = m_BgmSources[i];
                if (!src.isPlaying || src.clip != bgmClip) continue;
                return true;
            }
            return false;
        }
        ...
        public void StopSE() {
            if (!CheckExistSESource()) return;

            m_SeSource.Stop();
        }

        public bool IsPlayingSE() {
            if (!CheckExistSESource()) return false;
        ...
        private AudioSource[] GenerateAudioSourceList(int maxNum, AudioMixerGroup mixer, string listName) {
            ...
            for (int i = 0; i < transform.childCount; i++) {
                ...
                if (child.name == listName) Destroy(child);
            }
        ...
        private void PlayBGMClip(int idx, AudioClip clip) {
            if (!CheckExistBGMSource(idx)) return;
        ...
        private bool CheckExistBGMSource(int idx) {
            ...
            if (Common.Utility.ArrayUtility.IsOutOfArray(m_BgmSources, idx)) {
#if DEBUG_ON
        ...
        public class AudioManagerEditor : Editor {
            ...
            private static bool GenerateScript(List<AudioInfo> infos) {
                try {
                    ...
                    foreach (var info in infos.Select(c => new { var = RemoveInvalidChars(c.Name), val = c.Name })) {
