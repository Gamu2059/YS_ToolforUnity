...
namespace YS_Tool.IO {
    ...
    public sealed class FileIOManager : Singleton<FileIOManager> {
        ...
        /// 指定されたファイルが存在している場合は上書き、存在していない場合は新規作成します。
        /// </summary>
        public void EncryptSave(string fullPath, [NotNull] BaseDataSerializer dataSerializer, Action onSuccess = null, Action onFailure = null) {
            lock (_lockObject) {
                try {
                    if (File.Exists(fullPath)) {
                        File.Delete(fullPath);
        ...
        /// 指定されたファイルが存在していない場合は例外を発生させます。
        /// </summary>
        public void EncryptLoad(string fileFullPath, [NotNull] BaseDataSerializer dataSerializer, Action onSuccess = null, Action onFailure = null) {
            lock (_lockObject) {
                try {
                    if (!File.Exists(fileFullPath)) {
                        throw new IOException(string.Format("指定されたファイルが存在しません！ 指定されたファイル = {0}", fileFullPath));
